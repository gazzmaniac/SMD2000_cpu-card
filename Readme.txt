...
The card itself incorporates the A500_8MB_ide2 project for autobooting IDE and 8 mb fast ram.
...
Modified A500_8MB_ide2 IDE interface so cable select works.  See section below.
...
There is no need for it to look rough; mine has the resistors are inside heatshrink inside a DB25 shell, and it terminates in a header - for an extra 10 minutes work it looks bought.  I also cut up a USB cable and soldered headers to the +5V line so I could use the XP box's USB to supply power to the board while programming (there is already GND from the parallel port).
I used a PC with an LPT port running WindowsXP to program it.  


Compiling:
The A500_8MB_ide2 project didn't make the compiled *.jed file available to program the CPLD.  It's a bit of a stuff around if you haven't compiled a *.jed file before, and there isn't great documentation about how to do it so I have compiled it and included it in this package.  The *.jed file is not my work, it's the work of the author OlegMishin; all I did was compile it.
...
Open the file /firmware/v100/A500_8MB_ide in Quartus II and compile for 7128STC100 (they are equivalent parts, refer POF2JED application note).
This will create a bunch of files in /firmware/v100/output_files; you are interested in the file A500_8mb_ide2.pof.  

Then download POF2JED from:
https://www.microchip.com/en-us/development-tool/POF2JED
You'll need to register for an account, it's free.
Install it on your XP box.
...
Large Drives
*-----------------------------------------------------------------------------*
The Oktapussy driver you just flashed to the EPROM is limited to 4 gb drives (even using OS3.2), so you need to load the latest Oktapussy driver from disk.  If you want to use AmigaOS 1.3 you can't fix this problem, you need KS2.1 or above.
